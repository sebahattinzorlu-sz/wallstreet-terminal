...
# CSS: DOM Sıçramalarını Engelleyen Kararlı Matris Tasarımı
st.markdown("""
    <style>
...
# --- 2. YAPAY ZEKÂ KENDİ KENDİNE ÖĞRENME VE AKILLI BELLEK KATMANI ---
...
def kurumsal_hisse_tarayici(hisse_kodu):
    st.session_state["tarama_sayisi"] += 1
    try:
        ...
        son = gecmis.iloc[-1]
        fiyat = son['Close']
        rsi = round(son['RSI_14']) if 'RSI_14' in son and not pd.isna(son['RSI_14']) else 50
        ...
        sma50_deger = son.get('SMA_50', fiyat)
        ws_puan, ws_tavsiye, ws_aciklama = wall_street_karar_mekanizmasi(fiyat, rsi, sma50_deger, buyume, marj)
        
        return {
            "Fiyat": fiyat, "RSI": rsi, "Destek": son.get('BBL_20_2.0', fiyat * 0.95),
            "Direnç": son.get('BBU_20_2.0', fiyat * 1.05), "SMA50": sma50_deger, "Temel": temel,
            "WS_Puan": ws_puan, "WS_Tavsiye": ws_tavsiye, "WS_Aciklama": ws_aciklama
        }
    except Exception as genel_hata:
        sistem_ogrenme_kaydi(hisse_kodu, f"Genel motor hatası: {genel_hata}")
        return None

# --- 6. YAN PANEL: STRATEJİK ANA KONTROL MERKEZİ ---
with st.sidebar:
    st.title("🤖 ALPHA v53")
...
# --- 7. BÖLÜM 1-3: PİYASA REJİMİ & MAKRO DEĞERLENDİRME ---
if calisma_ekrani == "📊 BÖLÜM 1-3: Piyasa Rejimi & Makro Analiz":
    ...
    with c2:
        if sp_degisim < -0.05:
            st.error("📉 TREND YÖNÜ: BEARİSH / SATICILI")
        else:
            st.success("📈 TREND YÖNÜ: BULLİSH / ALICILI")
    with c3:
...
# --- 8. BÖLÜM 4-5: EKONOMİK TAKVİM & SEKTÖREL ROTASYON ---
...
# --- 9. BÖLÜM 6: İZLEME LİSTESİ VE SEÇİM TABANLI DİNAMİK ANALİZ ---
    ...
    st.caption("💡 Tablodan herhangi bir hisse satırına tıklayarak alttaki derinlemesine analiz merkezini güncelleyebilirsiniz.")
    
    izleme_listesi = ["HIMS", "RKLB", "ASTS", "OKLO", "SMCI", "IREN", "NVDA"]
    matris_listesi = []
    ...
    secilen_hisse = "HIMS"
    if secim_kutusu and "rows" in secim_kutusu.get("selection", {}) and secim_kutusu["selection"]["rows"]:
        secilen_satir_indeksi = secim_kutusu["selection"]["rows"][0]
        secilen_hisse = ana_df.iloc[secilen_satir_indeksi]["Hisse Kodu"]
        
    st.divider()
    
    st.markdown(f'<div class="section-title">💡 STRATEJİK ODAK: {secilen_hisse} DERINLEMESINE ANALIZ MERKEZİ</div>', unsafe_allow_html=True)
    ...
    if odak_verisi is not None:
        ...
        with hk2:
            ...
            with tabs[0]:
                if secilen_hisse == "HIMS":
                    st.markdown("* **GLP-1 Ürün Grubu:** Kilo verme dikeyindeki talep dengesi izlenmektedir.\n* **Düzenleme Riskleri:** FDA mevzuat adımları takip ediliyor.")
                else:
...
# --- 10. BÖLÜM 7-10: SIRA DIŞI İŞLEMLER VE AL-SAT STRATEJİLERİ ---
    ...
    firsat_tablosu = pd.DataFrame({
        ...
        "Hisse Senedi": ["HIMS", "AAPL", "NVDA", "RKLB", "IREN"],
    ...
    with g1:
        islem_hisse = st.selectbox("İşlem Gören Hisse:", ["HIMS", "RKLB", "ASTS", "OKLO", "IREN", "NVDA"])
    with g2:
